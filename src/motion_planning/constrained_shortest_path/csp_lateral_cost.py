...
import math
import re
from src.motion_planning.constrained_shortest_path.gurobi_license import options

class CSP_LATERAL_COST:
    ...
    def setup_model(self):
        ...
        with gp.Env(params=options) as env:
            ...
            self.model.setObjective(
                gp.quicksum(self.x[i, j] * (self.travel_time_cost[(i, j)] + 10*self.deviation_cost[(i,j)]) 
                            for i, j, _ in self._edges),
                GRB.MINIMIZE
            )
